...
class BookingReceiptScreen extends StatelessWidget {
  ...
  BookingReceiptScreen({super.key});
  ...
  Widget _buildReceiptContent(BuildContext context) {
    late Map<String, List<Map<String, dynamic>>> activeMenu;

    if (controller.selectedPackage.value == 'Custom Package' ||
        controller.isPackageEditing.value) {
      activeMenu = controller.menu;
    } else {
      activeMenu = controller.menuForPackage(
        controller.selectedPackage.value,
        controller.guests.value > 0 ? controller.guests.value : 1,
      );
    }

    // Safety check just in case menu is malformed
    if (!activeMenu.containsKey('Food Items')) {
      activeMenu['Food Items'] = [];
    }
    if (!activeMenu.containsKey('Services')) {
      activeMenu['Services'] = [];
    }
    ...
    return Column(
      ...
      children: [
        ...
        // Food Items
        _buildFoodItemsSection(activeMenu['Food Items']!),
        const SizedBox(height: 20),

        // Additional Services
        _buildServicesSection(activeMenu['Services']!),
        const SizedBox(height: 20),
      ...
      child: Column(
        children: [
          ...
          _buildSubtotalRow(
            ...
            controller.totalAmount - controller.vat,
          ),
